...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public class TintedCellsLayer : INotifyActorDisposing, ITick, ITickRender
	{
		...
		public void IncreaseLevel(CPos cell, int level, int maxLevel)
		{
			...
			var newLevel = tiles[cell].Level + level;
			if (newLevel > maxLevel)
				newLevel = maxLevel;

			// the given weapon can't saturate the cell anymore.
			if (tiles[cell].Level > newLevel)
				return;

			tiles[cell].SetLevel(newLevel);

			dirty.Add(cell);
		}

		/// <summary>Returns true when it decays.</summary>
		public bool Decay(TintedCell tintedCell, int updateDelay)
		{
			tintedCell.Ticks--;
			if (tintedCell.Ticks > 0)
				return false;

			tintedCell.Ticks = updateDelay;

			var decreaseLevel = FalloutScale * tintedCell.Level / 1000;

			// has to be decreased by at least 1 so that it disappears eventually.
			if (decreaseLevel < 1)
				decreaseLevel = 1;

			tintedCell.SetLevel(tintedCell.Level - decreaseLevel);
			return true;
		}
